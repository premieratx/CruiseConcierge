...
async function createBlogPosts() {
  try {
    ...
    console.log(`✅ Author created: ${author.name} (${author.id})`);
    ...
    const categories = await Promise.all([
      storage.createBlogCategory({
        ...
        description: "Tips, guides, and ideas for unforgettable bachelor parties in Austin"
      }),
      storage.createBlogCategory({
        name: "Bachelorette Parties",
        slug: "bachelorette-parties",
        description: "Everything you need to plan the perfect bachelorette party in Austin"
      }),
      storage.createBlogCategory({
        name: "Austin Travel",
        slug: "austin-travel",
        description: "Your guide to Austin's best attractions, dining, and entertainment"
      }),
      storage.createBlogCategory({
        name: "Lake Travis",
        slug: "lake-travis",
        description: "Discover the beauty and excitement of Lake Travis party boat adventures"
      }),
      storage.createBlogCategory({
        name: "Party Planning",
        slug: "party-planning",
        description: "Expert tips and strategies for planning memorable celebrations"
      })
    ]);
    ...
    const tags = await Promise.all([
      storage.createBlogTag({ name: "Austin", slug: "austin", description: "Austin, Texas" }),
      storage.createBlogTag({ name: "Lake Travis", slug: "lake-travis", description: "Lake Travis water activities" }),
      storage.createBlogTag({ name: "Bachelor Party", slug: "bachelor-party", description: "Bachelor party planning" }),
      storage.createBlogTag({ name: "Bachelorette Party", slug: "bachelorette-party", description: "Bachelorette party ideas" }),
      storage.createBlogTag({ name: "Party Boat", slug: "party-boat", description: "Party boat cruises" }),
      storage.createBlogTag({ name: "Party Planning", slug: "party-planning", description: "Event planning tips" }),
      storage.createBlogTag({ name: "Travel Guide", slug: "travel-guide", description: "Travel guides and tips" }),
      storage.createBlogTag({ name: "Monthly Guide", slug: "monthly-guide", description: "Month-specific planning guides" })
    ]);
    ...
    async function addPostRelations(postId: string, categoryNames: string[], tagNames: string[]) {
      const categoryIds = categoryNames.map(name => categories.find(c => c.name === name)?.id).filter(Boolean) as string[];
      const tagIds = tagNames.map(name => tags.find(t => t.name === name)?.id).filter(Boolean) as string[];
    ...
    // ==========================================
    // PART 2: CREATE 5 MAIN BLOG POSTS
    // ==========================================
    
    console.log('\n📝 Creating main blog posts...');
    ...
    // Due to character limits, I'll create a continuation script for blogs 4-17
    console.log('\n✅ Phase 1 complete: Created 3 main blog posts');
    console.log('📊 Summary so far:');
    console.log(`   - 1 author: ${author.name}`);
    console.log(`   - ${categories.length} categories`);
    console.log(`   - ${tags.length} tags`);
    console.log(`   - 3 blog posts published`);
    console.log('\n🔄 Continue with remaining 14 blog posts...');
