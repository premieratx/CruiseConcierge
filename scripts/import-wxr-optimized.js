// scripts/import-wxr-optimized.js
// OPTIMIZED WordPress import - fixes O(n²) performance issue
// Appends to index during import, sorts once at the end
...
// OPTIMIZED: Simple append (no sorting during import)
async function pushToIndex(indexKey, value) {
...
(async ()=>{
  console.log("🚀 Starting OPTIMIZED WordPress import...");
  ...
  let imported = 0;
  let enhanced = 0;
  const postsForSorting = []; // Collect posts with dates for final sort
  ...
  for (const it of items) {
    ...
    // OPTIMIZED: Just append to index (no sorting yet)
    await pushToIndex("index:posts", post.slug);
    await pushToIndex(`index:author:${author.slug}:posts`, post.slug);

    // Store for sorting later
    postsForSorting.push({ slug: post.slug, date: publishedAt });

    for (const c of cats) {
      await setIfMissing(`category:${c.slug}`, c);
      await pushToIndex(`index:category:${c.slug}:posts`, post.slug);
    }
    
    for (const t of post.tags) {
      await setIfMissing(`tag:${t.slug}`, t);
      await pushToIndex(`index:tag:${t.slug}:posts`, post.slug);
    }
  ...
  // OPTIMIZATION: Sort main index ONCE at the end
  console.log("\n📊 Sorting posts by date...");
  postsForSorting.sort((a, b) => {
    const dateA = a.date ? new Date(a.date).getTime() : 0;
    const dateB = b.date ? new Date(b.date).getTime() : 0;
    return dateB - dateA; // Newest first
  });
  const sortedSlugs = postsForSorting.map(p => p.slug);
  await db.set("index:posts", sortedSlugs);
  console.log(`✓ Sorted ${sortedSlugs.length} posts`);
