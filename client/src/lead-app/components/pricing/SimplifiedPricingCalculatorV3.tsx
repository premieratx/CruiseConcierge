...
import { Card, CardContent, CardHeader, CardTitle } from "@/lead-app/components/ui/card";
import { Label } from "@/lead-app/components/ui/label";
import { Slider } from "@/lead-app/components/ui/slider";
import { RadioGroup, RadioGroupItem } from "@/lead-app/components/ui/radio-group";
import { Button } from "@/lead-app/components/ui/button";
import { calculatePricing } from "@/lead-app/lib/pricing";
...
export const SimplifiedPricingCalculatorV3 = () => {
  const [guestCount, setGuestCount] = useState(10);
  const [discountPercent, setDiscountPercent] = useState("0");

  const calculateDisco = (pricePerPerson: number) => {
    const base = pricePerPerson * guestCount;
    const discount = base * (parseFloat(discountPercent) / 100);
    const subtotal = base - discount;
    const gratuity = subtotal * 0.20;
    // Xola fee is calculated on subtotal + gratuity
    const xolaFee = (subtotal + gratuity) * 0.03;
    const tax = subtotal * 0.0825;
    const total = subtotal + xolaFee + tax + gratuity;
    return { base, discount, subtotal, xolaFee, tax, gratuity, total, perPerson: total / guestCount };
  };
  ...
  const calculatePrivate = (dayOfWeek: number) => {
    ...
    const discount = pricing.subtotal * (parseFloat(discountPercent) / 100);
  ...
  return (
    <Card className="w-full max-w-[95vw] mx-auto">
      <CardHeader className="pb-1 pt-1.5">
        <CardTitle className="text-lg font-bold">Quick Pricing Calculator - Version 3</CardTitle>
      </CardHeader>
      <CardContent className="space-y-2 py-1">
        ...
        <div className="bg-accent/30 rounded-lg p-2 border-2 border-border">
          <Label className="text-sm font-bold mb-1 block">Discount Percentage</Label>
          <RadioGroup value={discountPercent} onValueChange={setDiscountPercent} className="flex flex-wrap gap-3">
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="0" id="discount-0" />
              <Label htmlFor="discount-0" className="text-sm cursor-pointer">0%</Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="25" id="discount-25" />
              <Label htmlFor="discount-25" className="text-sm cursor-pointer">25%</Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="30" id="discount-30" />
              <Label htmlFor="discount-30" className="text-sm cursor-pointer">30%</Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="35" id="discount-35" />
              <Label htmlFor="discount-35" className="text-sm cursor-pointer">35%</Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="40" id="discount-40" />
              <Label htmlFor="discount-40" className="text-sm cursor-pointer">40%</Label>
            </div>
          </RadioGroup>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <h3 className="text-xl font-bold">ATX Disco Cruise Pricing</h3>
            <Button className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold">Book Now</Button>
          </div>
                  ...
                  <div className="space-y-1 text-lg">
                    ...
                    {pricing.discount > 0 && (
                      <div className="flex justify-between text-green-600">
                        ...
                        <span className="font-semibold">-${pricing.discount.toFixed(0)}</span>
                      </div>
                    )}
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between text-base font-medium">
                      <span className="text-muted-foreground">Per Person (Subtotal)</span>
                      <span>${(pricing.subtotal / guestCount).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="bg-primary/10 rounded px-3 py-1.5 mt-1 text-center">
                      ...
                      <div className="text-3xl font-bold text-primary">${pricing.perPerson.toFixed(2)}</div>
                      {pricing.discount > 0 && (
                        <div className="text-base font-semibold text-green-600">
                          ${(pricing.discount / guestCount).toFixed(0)} saved / person
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <h3 className="text-xl font-bold">Private Cruise Pricing ({boatName})</h3>
            <Button className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold">Book Now</Button>
          </div>
          <div className="grid grid-cols-3 gap-1.5">
              ...
              return (
                <div key={slot.label} className="bg-accent/50 rounded-lg p-3 border-2 border-border">
                  ...
                  <div className="space-y-1 text-lg">
                    ...
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Subtotal</span>
                      <span className="font-semibold">${pricing.discountedSubtotal.toFixed(0)}</span>
                    </div>
                    <div className="flex justify-between text-base font-medium">
                      <span className="text-muted-foreground">Per Person (Subtotal)</span>
                      <span>${(pricing.discountedSubtotal / guestCount).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Fees+Tax+Tip</span>
                      <span className="font-semibold">${(pricing.xolaFee + pricing.tax + pricing.gratuity).toFixed(0)}</span>
                    ...
                    <div className="bg-accent/30 rounded px-3 py-1.5 mt-1 text-center">
                      <div className="text-base font-semibold">PER PERSON</div>
                      <div className="text-3xl font-bold">${perPerson.toFixed(2)}</div>
                      {pricing.discount > 0 && (
                        <div className="text-base font-semibold text-green-600">
                          ${(pricing.discount / guestCount).toFixed(0)} saved / person
                        </div>
