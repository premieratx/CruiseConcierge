...
export interface IStorage {
  ...
  createPartnerApplication(data: InsertPartnerApplication): Promise<PartnerApplication>;
  getPartnerApplications(): Promise<PartnerApplication[]>;
  getPartnerApplicationByEmail(email: string): Promise<PartnerApplication | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  constructor() {
    ...
    // OLD SEED LOGIC REMOVED - Database seeding disabled for Lovable migration
  ...
  async createPartnerApplication(data: InsertPartnerApplication): Promise<PartnerApplication> {
    const result = await db.insert(partnerApplications).values({
      ...data,
      orgId: "org_demo",
      status: "pending",
    }).returning();
  ...
  async getPartnerApplicationByEmail(email: string): Promise<PartnerApplication | undefined> {
    const result = await db.select().from(partnerApplications).where(eq(partnerApplications.email, email)).limit(1);
    return result[0];
  }
