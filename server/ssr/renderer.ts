 ...
 */
function generateSSRNavigation(): string {
  return `
...
/**
 * Generate static SSR footer for crawlers
 * CRITICAL: Provides additional internal links for SEO
 */
function generateSSRFooter(): string {
  ...
  <footer style="background: #1e293b; color: #f8fafc; padding: 2rem; margin-top: 3rem;">
    <div style="max-width: 1200px; margin: 0 auto;">
      ...
      <p style="font-size: 0.75rem; color: #64748b; margin-top: 0.5rem;">&copy; ${currentYear} Premier Party Cruises. All rights reserved.</p>
    </div>
  </footer>`;
}

/**
 * Wrap SSR content with navigation and footer for proper internal linking
 * This ensures crawlers see navigation on all pages
 */
function wrapWithSSRNavigation(content: string): string {
  return generateSSRNavigation() + content + generateSSRFooter();
}
...
window.__vite_plugin_react_preamble_installed__ = true
    ...
    // CRITICAL SEO FIX: Wrap SSR content with navigation and footer for internal linking
    // This ensures crawlers see proper navigation links on ALL pages including home links
    // Without this, SEMRush reports "only one incoming link" for the home page
    const ssrNav = generateSSRNavigation();
    const ssrFooter = generateSSRFooter();
    
    // Extract content from root div and wrap with navigation
    // Pattern: <div id="root">CONTENT</div> -> nav + <div id="root">CONTENT</div> + footer (as siblings)
    // SSR content goes as siblings to allow CSS hiding after React hydrates
    const wrappedContent = ssrContent.replace(
      /^<div id="root">(.*)<\/div>$/s,
      `<div id="root"></div><div class="ssr-content">${ssrNav}$1${ssrFooter}</div>`
    );
