...
- **Xola Integration**: Embedded slide-out checkout - all "Book Now" buttons (desktop header, mobile header, mobile bottom nav, mobile menu) use `xola-embedded-checkout` class with `data-button-id="691574bd162501edc00f151a"`. Xola's checkout.js script automatically initializes these elements to display a slide-out panel from the right side showing "Select an Activity" with cruise options. Initialization handled by `client/src/services/xola.ts` with production-proven `Xola.Controls.reload()` and manual fallback via `addEventListener` calling `Xola.onClick(element)`.
